...
def scrapCaseFIWiktionary(word, case):
    ...
    header_end_index = r.text.find("</header>") + len("</header>")
    shortened_text = r.text[header_end_index:]
    
    
    root = ET.fromstring(shortened_text)
    _tr = root.findall(".//tr")
...
scrap("Inessive", "ine", "inessiivi", "inessive", range_a=10000)